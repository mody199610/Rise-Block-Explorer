...
module.exports = function (app) {
    function Active () {
        this.getActive = function (cb) {
            request.get({
                url : app.get('rise address') + '/api/delegates/?orderBy=rate:asc&limit=101',
                json : true
        ...
        this.getForged = function (delegate, cb) {
            request.get({
                url : app.get('rise address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + delegate.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getStandby = function (n, error, success) {
        ...
        request.get({
            url : app.get('rise address') + '/api/delegates/?orderBy=rate:asc' + '&limit='  + delegates.limit + '&offset=' + delegates.actualOffset,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
    ...
    function Registrations () {
        this.getTransactions = function (cb) {
            request.get({
                url : app.get('rise address') + '/api/transactions/?orderBy=t_timestamp:desc&limit=5&type=2',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getDelegate = function (tx, cb) {
            request.get({
                url : app.get('rise address') + '/api/delegates/get?publicKey=' + tx.senderPublicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getLatestVotes = function (error, success) {
        request.get({
            url : app.get('rise address') + '/api/transactions/?orderBy=t_timestamp:desc&limit=5&type=3',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    function LastBlock () {
        this.getBlock = function (cb) {
            request.get({
                url : app.get('rise address') + '/api/blocks?&orderBy=height:desc&limit=1',
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getDelegate = function (block, cb) {
            request.get({
                url : app.get('rise address') + '/api/delegates/get?publicKey=' + block.generatorPublicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getLastBlocks = function (params, error, success) {
        ...
        request.get({
            url : app.get('rise address') + '/api/blocks?orderBy=height:desc&generatorPublicKey=' + params.publicKey + '&limit=' + params.limit,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : err });
