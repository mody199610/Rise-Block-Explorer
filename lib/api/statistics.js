...
module.exports = function (app) {
    ...
    this.getBlocks = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('rise address') + blocks.url(offset, limit),
                    json : true
    ...
    this.getLastBlock = function (error, success) {
        ...
        request.get({
            url: app.get('rise address') + blocks.url(0, 1),
            json : true
        }, function (err, resp, body) {
            if (err || resp.statusCode !== 200) {
    ...
    function Peers (locator) {
        ...
        var osBrands = { 'darwin' : 0, 'linux' : 1, 'win' : 2 };
    ...
    this.getPeers = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('rise address') + peers.url(offset, limit),
                    json : true
                }, function (err, resp, body) {
                    if (err || resp.statusCode !== 200) {
                        return next(err || 'Response was unsuccessful');
