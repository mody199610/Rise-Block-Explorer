...
module.exports = function (app, api) {
    ...
    this.getPriceTicker = function (error, success) {
        if (exchange.RISEBTC) {
            if (exchange.RISEBTC == '~') {
                return success({ success : true, btc_usd : (0).toFixed(8), rise_btc : (0).toFixed(8), rise_usd : (0).toFixed(2) });
            } else {
                return success({ success : true, btc_usd : exchange.BTCUSD, rise_btc : exchange.RISEBTC, rise_usd : exchange.RISEUSD(1) });
            }
        } else {
