...
module.exports = function (app) {
    function Account () {
        ...
        this.getAccount = function (address, cb) {
            request.get({
                url : app.get('rise address') + '/api/accounts?address=' + address,
                json : true
                ...
                if (err || response.statusCode !== 200) {
                    ...
                    account.usd = exchange.RISEUSD(account.balance);
        ...
        this.getDelegate = function (account, cb) {
            ...
            request.get({
                url : app.get('rise address') + '/api/delegates/get?publicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
        ...
        this.getForged = function (account, cb) {
            ...
            request.get({
                url : app.get('rise address') + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + account.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getTopAccounts = function (query, error, success) {
        ...
        request.get({
            url : app.get('rise address') + '/api/accounts/top?&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                ...
                async.forEach(body.accounts, function (a, cb) {
                    a.usd = exchange.RISEUSD(a.balance);
